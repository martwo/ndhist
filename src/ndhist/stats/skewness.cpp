...
)
{
    ...
    )
    {
        ...
        ss << "The axis and weight data types must be POD types. Non-POD types "
           << "are not supported by the skewness function!";
    ...
    #define NDHIST_MULTPLEX(r, seq)                                             \
        ...
        {                                                                       \
            BOOST_PP_SEQ_ELEM(0,seq) skewness = ::ndhist::stats::detail::calc_axis_skewness_impl<BOOST_PP_SEQ_ELEM(0,seq), BOOST_PP_SEQ_ELEM(1,seq)>(h, axis_idx);\
            return bp::object(skewness);                                             \
        }
    ...
    ss << "The combination of axis value type and weight value type of this "
       << "ndhist object is not supported for the skewness function!";
    throw TypeError(ss.str());
}
...
)
{
    if(axis != bp::object())
    {
        // A particular axis is given. So calculate the skewness only for that
        // axis and return a scalar.
        intptr_t const axis_idx = bp::extract<intptr_t>(axis);
        return detail::calc_axis_skewness(h, axis_idx);
    }

    // No axis was specified, so calculate the skewness for all axes.
    intptr_t const nd = h.get_nd();

    // Return a scalar value if the dimensionality of the histogram is 1.
    if(nd == 1)
    {
        return detail::calc_axis_skewness(h, 0);
    }

    // Return a tuple holding the skewness values for each single axis.
    std::vector<bp::object> skewnesses;
    skewnesses.reserve(nd);
    for(intptr_t i=0; i<nd; ++i)
    {
        skewnesses.push_back(detail::calc_axis_skewness(h, i));
    }
    return boost::python::make_tuple_from_container(skewnesses.begin(), skewnesses.end());
}
