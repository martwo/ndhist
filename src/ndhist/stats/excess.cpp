...
)
{
    ...
    )
    {
        ...
        ss << "The axis and weight data types must be POD types. Non-POD types "
           << "are not supported by the excess function!";
    ...
    #define NDHIST_MULTPLEX(r, seq)                                             \
        ...
        {                                                                       \
            BOOST_PP_SEQ_ELEM(0,seq) excess = ::ndhist::stats::detail::calc_axis_excess_impl<BOOST_PP_SEQ_ELEM(0,seq), BOOST_PP_SEQ_ELEM(1,seq)>(h, axis_idx);\
            return bp::object(excess);                                             \
        }
    ...
    ss << "The combination of axis value type and weight value type of this "
       << "ndhist object is not supported for the excess function!";
    throw TypeError(ss.str());
}
...
)
{
    if(axis != bp::object())
    {
        // A particular axis is given. So calculate the excess only for that
        // axis and return a scalar.
        intptr_t const axis_idx = bp::extract<intptr_t>(axis);
        return detail::calc_axis_excess(h, axis_idx);
    }

    // No axis was specified, so calculate the excess for all axes.
    intptr_t const nd = h.get_nd();

    // Return a scalar value if the dimensionality of the histogram is 1.
    if(nd == 1)
    {
        return detail::calc_axis_excess(h, 0);
    }

    // Return a tuple holding the excess values for each single axis.
    std::vector<bp::object> excesses;
    excesses.reserve(nd);
    for(intptr_t i=0; i<nd; ++i)
    {
        excesses.push_back(detail::calc_axis_excess(h, i));
    }
    return boost::python::make_tuple_from_container(excesses.begin(), excesses.end());
}
